...
public class Diablo {
    public static void main(String[] args) throws SQLException {
        ...
        PreparedStatement query = connection.prepareStatement("SELECT user_name, first_name," +
                "last_name, (SELECT COUNT(*) FROM users_games WHERE user_id = u.id) AS games_count FROM users AS u" +
                " WHERE user_name = ?");
        ...
        if (result.next()) {
            ...
            System.out.printf("User: %s%n%s %s has played %d games",
                    dbUsername, dbFirstName, dbLastName, dbGamesCount, dbGamesCount);
        } else {
